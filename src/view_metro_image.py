...
def add_brightness_mask(img_arr, sat_low, sat_high, colors=[[0, 0, 255], [255, 0, 0]]):
    ...
    # color pixels below brightness threshold with first color
    arr_3_channel[sat_mask_low] = colors[0]
    
    # mask pixels above brightness threshold
    sat_mask_high = img_arr > sat_high
    
    # color pixels above brightness threshold with second color
    arr_3_channel[sat_mask_high] = colors[1]
    
    return arr_3_channel

    
def plot_metro_image(image_path, sat_low=80, sat_high=200):
    ...
    # Plot the image
    plt.imshow(img_3_channel, cmap='gray')
...
def main():
    ...
    plot_metro_image(args.image_path, args.low, args.upper)
